...
def export_model_as_onnx(model, onnx_model_path: str, use_obs_array: bool = False, obs_keys=None):
    ...
    torch.onnx.export(
        ...
        dynamo=False # SFr: is now True since PyTorch 2.9 but the overall code does not match dynamo=True; hence; I set it to False
    )
...
#        action_onnx, state_outs = ort_sess.run(None, {**obs, "state_outs_orig": np.array([0.0], dtype=np.float32)})
